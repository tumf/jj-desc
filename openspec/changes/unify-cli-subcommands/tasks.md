...
- [ ] 2. コマンド実行ロジックの統合 (`src/commands/mod.rs`)
...
- [ ] 5. 仕様の更新 (`openspec/specs/backfill-descriptions/spec.md`)
...
- [ ] `cargo test` が全て通過
- [ ] `cargo clippy --all-features -- -D warnings` が警告なし
- [ ] `jj-desc --help` が正しく表示される
- [ ] `jj-desc -r @` が動作する
- [ ] `jj-desc` (引数なし) がデフォルト動作する

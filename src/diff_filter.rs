...
impl FilteredDiff {
    /// Create a new FilteredDiff from content
...
mod tests {
    ...
    fn test_filtered_diff_new() {
        ...
        let diff = FilteredDiff::new(content.clone(), original_size);
    ...
    fn test_filtered_diff_line_count() {
        let content = "line1\nline2\nline3".to_string();
        let diff = FilteredDiff::new(content, 100);
        assert_eq!(diff.line_count(), 3);
    }

    #[test]
    fn test_filtered_diff_has_exclusions() {
        let mut diff = FilteredDiff::new("content".to_string(), 100);
